#!/usr/bin/env node
/**
 * Controlled staging-only runner for atomic device activation.
 *
 * Usage:
 *   node tools/run-auto-device-activation-staging-migration.js --dry-run
 *   node tools/run-auto-device-activation-staging-migration.js --apply
 *
 * The complete additive migration runs inside one transaction. This tool
 * never invokes `supabase db push` and never prints private environment data.
 */
...
const MIGRATION = path.join(ROOT, 'supabase', 'migrations', '20260723_auto_approve_device_activation.sql');
...
const wrapperPath = path.join(os.tmpdir(), `plotmap-auto-activation-${process.pid}-${Date.now()}.sql`);
